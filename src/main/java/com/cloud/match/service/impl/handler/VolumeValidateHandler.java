...
@Component
public class VolumeValidateHandler implements ValidateHandler {
    ...
    public void handle(Order order,  MatchEngine engine) {
        ...
        // 处理自己核心逻辑
        if (order.getPositionAmount().compareTo(BigDecimal.ZERO) <= 0) {
            throw new ValidationException("订单数量不允许小于0");
        }
